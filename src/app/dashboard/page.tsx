...
import { Server, Cpu, HardDrive, Activity, LogOut, Plus, ArrowUpDown, AlertTriangle } from "lucide-react"
    ...
    if (error) {
      return (
        <div className="bg-red-900/20 border border-red-500/30 p-6 rounded-lg text-center text-red-300">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Failed to Load Instances</h3>
    ...
    if (instances.length === 0) {
      return (
        <div className="text-center text-slate-400 py-16 bg-slate-800/30 rounded-lg">
          ...
          <p>As soon as a server with Telegraf starts reporting, it will appear here.</p>
        </div>
      );
    }
            ...
            <Card className="bg-slate-800/50 border-slate-700 backdrop-blur hover:bg-slate-800/70 transition-all cursor-pointer h-full flex flex-col">
              <CardHeader>
                <div className="flex justify-between items-start mb-2">
                  <CardTitle className="text-white text-lg truncate" title={instance.name}>{instance.name}</CardTitle>
              ...
              <CardContent className="space-y-4 flex-grow flex flex-col justify-end">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="text-slate-400">
                      {instance.ram_used.toFixed(0)} / {instance.ram_total.toFixed(0)} MB
                    </span>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="text-slate-400">
                      {instance.storage_used.toFixed(1)} / {instance.storage_total.toFixed(1)} GB
                    </span>
                  </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-400">Live overview of your VDS infrastructure</p>
          </div>
